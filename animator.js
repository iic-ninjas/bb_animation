...
Animator.lerp = function(t, min, max) {
  return (1-t)*min + t*max;
};

Animator.map = function(val, valMin, valMax, targetMin, targetMax) {
  return targetMin + (targetMax - targetMin) * ((val - valMin) / (valMax - valMin));
};

Animator.partitionAnimation = function(val, min, max) {
  return Animator.map(Math.max(min, Math.min(val, max)), min, max, 0, 1);
};
